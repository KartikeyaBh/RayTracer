...
glm::dvec3 GetDirection(const glm::vec3& P, TraceLight* trace_light, TraceScene scene)
{
    Light* scene_light = trace_light->light;
    if (PointLight* pointlight = scene_light->as<PointLight>())
    {
        return (trace_light->GetTransformPos() - P);
    }
    else if (DirectionalLight* dirlight = scene_light->as<DirectionalLight>())
    {
        return trace_light->GetTransformDirection();
    }
    return glm::dvec3(0, 0, 0);
}
glm::vec3 ShadowAttenuation(const glm::dvec3& P, TraceLight* trace_light, TraceScene scene)
{
    Light* scene_light = trace_light->light;
    if (PointLight* pointlight = scene_light->as<PointLight>())
    {
        glm::vec3 L = GetDirection(P, trace_light, scene);

        double tn;

        Ray r(P, glm::normalize(L));

        Intersection i;
        scene.Intersect(r, i);

        tn = i.t;
        double tlight = glm::length(L);

        if ( abs(tn) < tlight)
        {
            return i.GetMaterial()->Transmittence->GetColorUV(i.uv);
        }
        else
        {
...
glm::dvec3 Shade( TraceScene trace_scene, const Ray& r, Intersection& i )
{
    glm::dvec3 contribution = glm::dvec3(0,0,0);
    ...
    glm::dvec3 ke = mat->Emissive->GetColorUV(i.uv);
    ...
    for (auto j = trace_scene.lights.begin(); j != trace_scene.lights.end(); j++){
        ...
        L = glm::normalize(GetDirection(Q, trace_light, trace_scene)); //V - Q
        H = glm::normalize(V + L);
        NL = glm::dot(N, L);

        B = 1.0;
        if (NL < NORMAL_EPSILON * NORMAL_EPSILON)
            B = 0.0;
        if (PointLight* pointlight = scene_light->as<PointLight>())
        {
                float a = pointlight->AttenC.Get();
                float b = pointlight->AttenB.Get();
                float c = pointlight->AttenA.Get();
                float radius = glm::length(trace_light->GetTransformPos() - Q);

                float distance_attenuation = std::min((float)1.0, 1/(a + b * radius + c * radius * radius));
                glm::vec3 R = N * (float)2.0 * NL - L;
                glm::vec3 V = - r.direction;
                // B factor
                float B = 1.0;
                if (NL < 0.00001) { B = 0.0; }

                auto temp1= ShadowAttenuation(Q, trace_light, trace_scene);
                auto temp2 = distance_attenuation;
                auto temp3 = pointlight->Color.GetRGB();
                auto temp4 = B;
                auto temp5 = (kd * NL +
                              ks * (float)pow(glm::dot(N, H), std::max(shininess, 0.00001)));
                contribution += kd * pointlight->Ambient.GetRGB() + ShadowAttenuation(Q, trace_light, trace_scene) * distance_attenuation * scene_light->Color.GetRGB() * B * (kd * NL +
                            ks * (float)pow(glm::dot(N, H), std::max(shininess, 0.00001)));
        }
        else {
            float B = 1.0;
            if (NL < 0.00001) { B = 0.0; }
            DirectionalLight* dirlight = scene_light->as<DirectionalLight>();
            contribution += kd * dirlight->Ambient.GetRGB() + ShadowAttenuation(Q, trace_light, trace_scene) * (float)1.0
                        * scene_light->Color.GetRGB() * B * (kd * NL + ks * (float)pow(glm::dot(N, H), std::max(shininess, 0.00001)));
        }
    }


    return ke + contribution;
}
const static int n_air = 1.0003;
bool checkTotalInternal(const Ray &r, Intersection &i){
    glm::vec3 dir = r.direction;
    if(dot(i.normal,dir) > 0.0f){
        double incidentAngle = dot(i.normal, dir);
        double ratioIndex = i.GetMaterial()->IndexOfRefraction;;
        if((1.0f - (ratioIndex*ratioIndex*(1.0f - incidentAngle * incidentAngle)))<0.0f)
            return true;}
return false;}

bool updateRefractionParams(const Ray& r, Intersection& i, glm::vec3& reflectedDirectionSi,
                            glm::vec3& refractedDirectionSt, glm::vec3& refractedDirectionCt, glm::vec3& refractedDir){
    Material* mat = i.GetMaterial();
    glm::vec3 kd = mat->Diffuse->GetColorUV(i.uv);
    glm::vec3 ks = mat->Specular->GetColorUV(i.uv);
    ...
    float index_of_refraction = mat->IndexOfRefraction;

    if((kt[0] <= 0.0f&&kt[1]<=0.0f&&kt[2]<=0.0f) || checkTotalInternal(r,i))
        return false;

    if(dot(i.normal, dir) < 0.0f){
        float ratioIndex = index_of_refraction;
        ratioIndex = 1.0f/(float)ratioIndex;
        refractedDirectionSt = ratioIndex * reflectedDirectionSi;
        float temp = dot(refractedDirectionSt, refractedDirectionSt);
        if(temp>1)
            temp = 0;
        refractedDirectionCt = (-1.0f * i.normal) * std::sqrt((1.0f - temp));
    } else if( dot(i.normal, dir) > 0.0f) {
        float ratioIndex = index_of_refraction;
        refractedDirectionSt = ratioIndex * reflectedDirectionSi;
        double temp = dot(refractedDirectionSt,refractedDirectionSt);
        if(temp>1)
            temp = 0;
        refractedDirectionCt = i.normal * (float)std::sqrt((1.0f - temp));}

    refractedDir = refractedDirectionSt + refractedDirectionCt;
    refractedDir= glm::normalize(refractedDir);
    return true;}

void updateReflectionParams(const Ray& r, Intersection& i,glm::vec3& reflectedDirectionCi, glm::vec3& reflectedDirectionSi, glm::vec3& reflectedDir){
     glm::vec3 dir = r.direction;
    reflectedDirectionCi = ((-dir)*i.normal)*i.normal;
    reflectedDirectionSi = reflectedDirectionCi + dir;
...
{
    Intersection i;
    if (depth > settings.max_depth) return glm::dvec3(0.0, 0.0, 0.0);
    if (debug_camera) {
    ...
    if (trace_scene.Intersect(r, i)) {     
        ...
        glm::vec3 intensity = kd;

        intensity = Shade(trace_scene, r, i);
        ...
        {
            glm::vec3 V = - r.direction;
            glm::vec3 N = glm::normalize(i.GetTrueNormal());
            float NVv = dot(N, V);
            if (NVv < 0)  N = -N;

            glm::vec3 direction = r.direction;
            glm::vec3 normalizedD = glm::normalize(direction);


            //reflection
            Ray r_reflect(r.at(i.t), glm::normalize(normalizedD - 2 * glm::dot(normalizedD, N)*N));


             intensity += ks * TraceRay(r_reflect, depth+1, RayType::reflection, debug_camera); //1st recursion
        }
        if (settings.refractions) {
            ...
            float cosQ = sqrt(1 - nr * nr * (1 - NVv * NVv));
            glm::vec3 T;
            ...
            auto new_r = Ray(r.at(i.t), glm::normalize(T));
            intensity = intensity + kt * TraceRay(new_r,depth + 1, RayType::refraction, debug_camera);
