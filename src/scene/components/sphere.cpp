...
bool Sphere::IntersectLocal(const Ray &r, Intersection &i)
{
    ...
    double a = glm::pow(glm::length(d), 2);
    double b = 2*glm::dot(d, p);
    double c = glm::pow(glm::length(p), 2) - 0.5 * 0.5;
    ...
    glm::vec3 dir = glm::normalize(r.at(t));

    // Calculate the UV coordinates
    // http://en.wikipedia.org/wiki/UV_mapping#Finding_UV_on_a_sphere
    i.uv =  glm::vec2(
        0.5f + atan2(dir[2], dir[0]) / 2 * M_PI,
        0.5f - asin(dir[1]) / M_PI
    );
    return true;
